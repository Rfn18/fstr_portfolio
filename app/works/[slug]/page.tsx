...
const technologies: PillsProps[] = [
  { list: ["TypeScript", "JavaScript", "React", "Next.js"] },
];

const techStack: PillsProps[] = [
  {
    list: [
      "TypeScript",
      "JavaScript",
...
const photoShowcase = [
  {
    ...
    desc: "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Numquam, exercitationem blanditiis. Itaque porro illo nesciunt cum est eum quod repudiandae, et earum mollitia enim eligendi.",
    role: "Fullstack Developer",
    responsibilities: [
      "Architected a scalable monorepo setup with Fastify, tRPC, and React",
      "Implemented a secure authentication system using JWT tokens and bcrypt encryption",
      "Developed RESTful APIs for real-time data processing and storage",
      "Created a responsive and user-friendly interface using React and Tailwind CSS",
      "Integrated computer vision libraries (OpenCV, DeepFace) for real-time face detection and recognition",
    ],
    impact: [
      "Reduced manual lead processing time by over 80% through background job automation.",
      "Improved data accuracy by implementing real-time validation and automated error correction.",
      "Enhanced user engagement through personalized recommendations and seamless user experience.",
    ],
  },
];
...
export default function ProjectPage() {
  const showcase = photoShowcase[0];
      ...
      <Container className="px-0 md:px-0 lg:px-0">
        ...
        >
          <div className="relative mt-6 mb-32 h-[50vh] w-full overflow-hidden rounded-xl sm:h-[60vh] md:h-[75vh] lg:mt-12 lg:h-[90vh] dark:bg-surface transition-colors duration-700 ease-in-out  ">
            <Image
              ...
              alt="CCTV Face Recognition project preview"
              fill
              priority
              sizes="100vw"
        ...
        >
              ...
              <div className="flex w-full flex-col items-start gap-3 lg:w-auto">
                ...
                <div className="flex flex-wrap gap-2">
                  {technologies.map((item, i) =>
                    item.list.map((tech, j) => (
                      <span key={`${i}-${j}`} className={PILL_CLASS}>
                        {tech}
                      </span>
                    )),
                  )}
          ...
          <div className="grid grid-cols-1 gap-3 px-3 py-8 sm:grid-cols-2 sm:gap-4 sm:px-4 md:gap-8 md:px-8">
            {showcase.project.secondary.map((src, j) => (
              <div
                key={j}
          ...
          >
            <VideoPlayer
              src="/videos/project/FACE_RECOGNIZE.mp4"
              label="Demo of the CCTV face recognition system"
